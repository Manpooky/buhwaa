...
const ImmigrationFormAnalyzer = () => {
  ...
  const translatedUploadMutation = useMutation({
    mutationFn: async (file: File) => {
      const formData = new FormData()
      formData.append("file", file)
  ...
  const handleDocumentUpload = async (files: File[], uploadType: string) => {
    ...
    try {
      const result = await uploadType === "initial" ? initialUploadMutation.mutateAsync(files[0]) : translatedUploadMutation.mutateAsync(files[0])
          ...
          <div className="flex items-center mb-8">
            ...
            <div
              ...
              <span className="ml-2 font-medium">
                Download and complete the document in your language
              </span>
            </div>
            <div className="flex-1 h-1 mx-4 bg-gray-200">
              <div
          ...
          {translatedDocument && (
            <div className="mb-8">
              <h2 className="text-xl font-semibold mb-4">
                Step 2: Download and fill out in your language
              </h2>
              ...
              {currentStep === 2 && (
                <FileUpload
                  ...
                  loading={translatedUploadMutation.isPending}
                />
              )}
            </div>
          )}
...
function FileUpload({
  ...
  handleDocumentUpload: (files: File[], uploadType: string) => void
    ...
    <div className="mb-8">
      ...
      <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
        ...
        {files && files.length > 0 && (
          <div
            className="mt-4 p-3 bg-gray-50 rounded flex items-center justify-center gap-2">
      ...
      {files && files.length > 0 && (
        <div className="flex w-full justify-end p-2">
          <button
            onClick={() => handleDocumentUpload(files, uploadType)}
            disabled={loading}
